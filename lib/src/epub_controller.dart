...
class EpubController {
  ...
  //! Added the totalIndexes property
  int get totalIndexes => _totalIndexes;
  int _totalIndexes = 0;
  ...
  Future<void> _loadDocument(Future<EpubBook> document) async {
    ...
    try {
      ...
      //! Updated the totalIndexes value
      _totalIndexes = _epubViewState!._paragraphs.length;

      loadingState.value = EpubViewLoadingState.success;
    } catch (error) {
      _epubViewState!._loadingError = error is Exception ? error : Exception('An unexpected error occurred');
